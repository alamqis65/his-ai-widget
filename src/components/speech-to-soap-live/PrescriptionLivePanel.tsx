...
interface Props {
  ...
  /** Panggil soapRecommendationEndpoint khusus resep (recommendation_type: 'PRESCRIPTION'). */
  onGenerateRecommendation?: () => void
  loading?: boolean
}

function PrescriptionLiveRow({
  item,
  checked,
  onToggle,
}: {
...
/**
 * PrescriptionLivePanel — versi Live dari prescriptionPanel.tsx (fitur lama).
 * File baru, bukan prop baru, supaya prescriptionPanel.tsx lama tidak
 * tersentuh. Tambahan dari versi lama: tombol "Rekomendasi" di header.
 */
export function PrescriptionLivePanel({ prescriptions, onSelectionChange, onGenerateRecommendation, loading }: Props) {
    ...
    <div class="suggestion-panel">
      <div class="suggestion-panel-header">
        ...
        <button
          ...
          disabled={loading}
          title="Minta rekomendasi resep dari AI"
        >
      ...
      {prescriptionItems.length === 0 ? (
        <p style="padding:10px 12px;margin:0;font-size:12px;color:var(--text-3)">
          Belum ada resep. Menunggu transkrip, atau klik "Rekomendasi".
        </p>
      ) : (
