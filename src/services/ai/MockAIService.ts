...
const MOCK_RESPONSES: Record<string, string> = {
  ...
  sapaan:
    'Halo! Selamat datang di HIS AI Assistant. Ada yang bisa saya bantu hari ini?',
  diagnosis:
...
function getResponse(message: string): string {
  ...
  if (lower.includes('halo') || lower.includes('hai') || lower.includes('selamat'))
    return MOCK_RESPONSES.sapaan
  if (lower.includes('diagnosis') || lower.includes('gejala') || lower.includes('sakit'))
    return MOCK_RESPONSES.diagnosis
  if (lower.includes('obat') || lower.includes('resep') || lower.includes('farmasi'))
    return MOCK_RESPONSES.obat
  if (lower.includes('soap') || lower.includes('catatan'))
    return MOCK_RESPONSES.soap
  if (lower.includes('icd') || lower.includes('kode'))
    return MOCK_RESPONSES.icd
